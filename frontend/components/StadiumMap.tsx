...
export default function StadiumMap({
  ...
  return (
    <svg
      ...
      aria-describedby="stadium-desc"
    >
      <title id="stadium-title">Live stadium map</title>
      <desc id="stadium-desc">
        A top-down schematic of the stadium. Each zone is a coloured badge whose fill indicates the
        current Crowd Flow Score: green (80 or higher) means healthy, amber (50 to 79) means watch,
        red (below 50) means action required. Size grows with occupancy. Thin dashed lines show
        pedestrian paths between neighbouring zones; animated dots represent people flowing along
        those paths — blue for gentle flow, amber for moderate, red for heavy. A pulsing outline
        marks any zone at a critical level. The pitch in the centre is green with a halfway line and
        centre circle.
      </desc>
      <defs>
      ...
      {!reducedMotion &&
        graph?.edges.map((e, i) => {
          ...
          const dir =
            (za?.outflow_per_min ?? 0) >= (zb?.outflow_per_min ?? 0)
        ...
        (() => {
          ...
          return (
            <g aria-label="Your current location" style={{ pointerEvents: "none" }}>
              ...
              <polygon
                points={`${p.x - 4},${p.y - 30} ${p.x + 4},${p.y - 30} ${p.x},${p.y - 22}`}
                fill="#0284c7"
              />
            </g>
          );
        ...
        return (
          ...
          >
            ...
            <rect
              ...
              rx={8}
              fill={col.soft}
              stroke={selected ? "#0284c7" : col.fill}
