...
export default function OpsActionFeed({ token }: { token: string | null }) {
  ...
  async function generate() {
    if (!token) { setErr("Log in first"); return; }
    setLoading(true); setErr(null);
    try {
      const p = await opsPlan(token);
      setPlan(p); setStatuses({}); setMessages({});
      toast.push("info", `Plan ready — ${p.actions.length} action${p.actions.length === 1 ? "" : "s"}`);
    } catch (e) {
  ...
  async function onApply(i: number) {
    ...
    try {
      const res = await applyOpsAction(token, {
        type: a.type, target: a.target, rationale: a.rationale,
      });
  ...
  return (
    <div className="panel" style={{ flex: 1, minHeight: 0 }}>
      ...
      <div className="info-banner" style={{ marginBottom: 10 }}>
        ...
        <span>Drag <b>Chaos</b> to force congestion, then click <b>Propose Actions</b>. Apply any action to dispatch it live — the map reacts on the next tick.</span>
      </div>
      ...
      <label>
        ...
        <input className="slider" type="range" min={0} max={100} value={chaos * 100}
               onChange={(e) => applyChaos(Number(e.target.value) / 100)} disabled={!token}/>
      </label>

      {err && <div className="alert-banner" role="alert">⚠ {err}</div>}

      {plan && (
        <div style={{ overflowY: "auto", marginTop: 10, paddingRight: 4 }}>
          <div className="card">
            <h3>Situation</h3>
            <div className="meta" style={{ marginTop: 4, color: "var(--text-2)" }}>{plan.situation}</div>
            <div className="meta"><b>Root cause:</b> {plan.root_cause}</div>
            <div className="meta">
              Confidence <b>{Math.round(plan.confidence * 100)}%</b> · engine: <span className="kbd">{plan.engine}</span>
            </div>
          </div>
          {plan.actions.map((a, i) => {
            const info = ACTION_LABEL[a.type] ?? { icon: "•", label: a.type };
            const status = statuses[i] ?? "pending";
            const cls = status === "applied" ? "card applied"
                      : status === "dismissed" ? "card dismissed"
                      : "card";
            return (
              <div key={i} className={cls}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
                  <h3><span aria-hidden>{info.icon}</span>&nbsp;{info.label} · <span style={{ color: "var(--muted)" }}>{a.target}</span></h3>
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    {status === "applied" && <span className="applied-badge">Applied ✓</span>}
                    {status === "failed"  && <span className="applied-badge" style={{ background: "#dc2626" }}>Failed</span>}
                    <span className="meta">ETA {a.eta_minutes}m</span>
                  </div>
                </div>
                <div className="meta" style={{ marginTop: 6, color: "var(--text-2)" }}>{a.rationale}</div>
                {messages[i] && (
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                  <button className="btn small"
                          onClick={() => onApply(i)}
                          disabled={status === "applying" || status === "applied" || status === "dismissed"}>
