...
export default function MapPage() {
  ...
  const zones = useMemo(() => tick?.zones ?? [], [tick]);
  const selected = useMemo(() => zones.find((z) => z.id === selectedId), [zones, selectedId]);
  const worst = useMemo(() => [...zones].sort((a, b) => a.score - b.score).slice(0, 5), [zones]);

  const latestAlert = tick?.alerts?.[0];
  const hotspots = useMemo(
    () =>
      zones
        .filter((z) => z.level === "critical" || z.level === "congested")
    ...
    <main className="layout">
      <section className="panel">
        ...
        <div className="legend" aria-hidden>
          <span className="tag">
            <span className="dot" style={{ background: "#16a34a" }} /> 80–100 Healthy
          </span>
          <span className="tag">
            <span className="dot" style={{ background: "#d97706" }} /> 50–79 Watch
          </span>
          <span className="tag">
            <span className="dot" style={{ background: "#dc2626" }} /> 0–49 Action
          </span>
      ...
      <aside style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
        ...
        <div className="panel" style={{ flex: 1, overflowY: "auto" }}>
          <div className="panel-head">
            ...
            {selected && (
              <button className="btn ghost small" onClick={() => setSelectedId(undefined)}>
                Clear
              </button>
            )}
          </div>
          {selected ? (
            ...
            worst.map((z) => <ZoneCard key={z.id} z={z} />)
          ) : (
