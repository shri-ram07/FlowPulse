...
describe("KIND_ICON", () => {
  it("covers every zone kind", () => {
    for (const kind of ["gate","seating","food","restroom","concourse","exit","merch"] as const) {
...
describe("edgeFlow", () => {
  const mk = (p: Partial<ZoneState>): ZoneState => ({
    id: "x", name: "X", kind: "food", capacity: 100, occupancy: 0,
    density: 0, inflow_per_min: 0, outflow_per_min: 0, wait_minutes: 0,
    trend: "steady", score: 100, level: "calm", x: 0, y: 0, ...p,
  });
