...
[![CI](https://img.shields.io/badge/CI-green-16a34a?logo=githubactions&logoColor=white)](.github/workflows/ci.yml)
[![Tests](https://img.shields.io/badge/tests-50%20passed-16a34a)](backend/tests)
[![Coverage](https://img.shields.io/badge/coverage-80%25-16a34a)](docs/BENCHMARKS.md)
...
[![Google ADK](https://img.shields.io/badge/Google%20ADK-Gemini%202.0%20Flash-4285f4?logo=google)](backend/agents)
...
flowchart TB
    ...
    subgraph AI ["AI stack"]
        ...
        GEM["Gemini 2.0 Flash<br/>function calling<br/>response_schema"]
...
| 2 | **Gemini 2.0 Flash** | Reasoning for 5 agents; function calling + `response_schema` + `system_instruction` | [`backend/agents/orchestrator_agent.py`](backend/agents/orchestrator_agent.py) |
...
| 4 | **Firebase Cloud Messaging (v1)** | Topic-based push for ops `push_notification` actions — OAuth 2 via runtime SA | [`backend/api/routes_fcm.py`](backend/api/routes_fcm.py) |
...
```mermaid
flowchart TB
    O["OrchestratorAgent<br/>(LlmAgent · Gemini 2.0 Flash)<br/>response_schema = OpsPlan"]
...
The Concierge (attendee-facing) composes Routing + Forecast the same way. Every agent's output is **Pydantic-typed** via `backend/agents/schemas.py`; Gemini is forced to emit JSON matching the schema.
...
```bash
# backend — 50 tests, coverage gate
...
CI on every push runs: **ruff · mypy strict on pure modules · pytest · tsc · Vitest · next build · gitleaks · Trivy container scan · SBOM generation · Lighthouse CI**. See [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
...
- [docs/BENCHMARKS.md](docs/BENCHMARKS.md) — measured engine / WebSocket / Gemini / Cloud Run / Locust numbers
