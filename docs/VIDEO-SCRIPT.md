...
| 00:50-01:30 | Live demo 1 — Fan view | Open `/chat`. Tap Gate A on map. Type "Quick snack?". Agent replies: "Food Court 5 — Score 85, 3-min walk." Hover over tool-chip so viewer sees `get_best_route()`. | *"The fan concierge, running on Google ADK with Gemini 3 Flash, answers by calling live tools on our Crowd Flow Engine. Every chip under an answer is a tool call — the agent can't make up numbers."* |
