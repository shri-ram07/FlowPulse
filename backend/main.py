...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:  # noqa: ARG001
    """Startup/shutdown hooks — warm the engine, start the sim, emit audit lines."""
    configure_logging()
    configure_tracing()
...
_cors_origins = [
    o.strip() for o in os.environ.get("FLOWPULSE_CORS_ORIGINS", _default_cors).split(",") if o.strip()
]
app.add_middleware(
    CORSMiddleware,
...
app.include_router(routes_csp.router)
app.include_router(ws.router)


@app.get("/api/health", tags=["meta"])
async def health() -> dict[str, int | str]:
    """Tiny liveness probe — returns zone + alert counts."""
    eng = get_engine()
