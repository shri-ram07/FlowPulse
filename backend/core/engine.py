...
from .scoring import congestion_level, crowd_flow_score, forecast
from .zone import Zone

EWMA_ALPHA = 0.3  # smoothing factor for inflow/outflow
...
class CrowdFlowEngine:
    ...
    async def enter(self, zone_id: str, n: int = 1) -> None:
        async with self._lock:
            ...
            z.occupancy = min(int(z.capacity * 1.3), z.occupancy + n)
    ...
    async def tick(self) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            for zid, z in self.zones.items():
                ...
                observed_in = self._entered_since_tick[zid] * (60.0 / dt)
                observed_out = self._exited_since_tick[zid] * (60.0 / dt)
    ...
    def _snapshot_payload(self, new_alerts: list[Alert], *, full: bool = False) -> dict[str, Any]:
        ...
        for z in self.zones.values():
            ...
            key = (state["occupancy"], state["score"], state["level"],
                   state["wait_minutes"], state["trend"])
    ...
    def _evaluate_risk(self, now: float) -> list[Alert]:
        fired: list[Alert] = []
        for z in self.zones.values():
            risk = z.density > 0.9 and z.inflow_rate > 2 * z.outflow_rate
            if risk:
                started = self._recent_risk_since.setdefault(z.id, now)
                # Fire after sustained risk for >15s (scaled for demo).
                if now - started > 15 and not self._already_alerted(z.id, now):
                    a = Alert(
                        id=str(uuid.uuid4()),
                        zone_id=z.id,
                        severity="critical",
        ...
        self.alerts = [a for a in self.alerts if now - a.ts < 300]
        return fired

    def _already_alerted(self, zone_id: str, now: float) -> bool:
        return any(a.zone_id == zone_id and now - a.ts < 60 for a in self.alerts)
