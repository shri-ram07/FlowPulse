...
async def test_halftime_moves_seating_to_concourses():
    ...
    seats_before = sum(eng.zones[z].occupancy
                       for z in ("seat_n", "seat_s", "seat_e", "seat_w"))
    conc_before = sum(eng.zones[z].occupancy
                      for z in ("con_n", "con_s", "con_e", "con_w"))

    sim._start_ts = _time.monotonic() - (2 * 120 + 30)  # 30s into halftime
    assert sim.state().phase == "halftime"
    await sim._step()

    seats_after = sum(eng.zones[z].occupancy
                      for z in ("seat_n", "seat_s", "seat_e", "seat_w"))
    conc_after = sum(eng.zones[z].occupancy
                     for z in ("con_n", "con_s", "con_e", "con_w"))
