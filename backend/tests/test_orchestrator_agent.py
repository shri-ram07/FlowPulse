...
async def test_comms_fallback_has_required_fields():
    d = fallback_comms("food_2", channel="push", severity="warn",
                       hint="Try Food Court 5 instead.")
...
@pytest.mark.asyncio
async def test_deterministic_plan_with_hot_zone_proposes_redirect_or_notify():
    eng = get_engine()
    # Saturate food_2 so safety_agent flags it.
    ...
    # Expect at least a push notification; a redirect may also fire if a
    # healthy food zone has headroom.
    assert "push_notification" in types or "monitor" in types
