...
def _zone(occupancy: int, capacity: int, inflow: float, outflow: float,
          kind: str = "food") -> Zone:
    return Zone(
        id="z", name="Z", kind=kind, capacity=capacity,
        x=0.0, y=0.0, occupancy=occupancy,
        inflow_rate=inflow, outflow_rate=outflow,
    )
...
def test_concourse_balanced_flow_stays_healthy(occupancy):
    ...
    z = _zone(occupancy, capacity=10000, inflow=10.0, outflow=10.0,
              kind="concourse")
