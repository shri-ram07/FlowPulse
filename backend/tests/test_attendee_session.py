...
def test_attendee_accepts_session_id(client: TestClient):
    r = client.post("/api/agent/attendee", json={
        "message": "where to eat?", "session_id": "tab-abc123",
    })
...
def test_session_cache_reuses_across_calls(monkeypatch):
    ...
    asyncio.get_event_loop().run_until_complete(
        adk_runtime.run_adk(runner, "u", "hi", session_id="sess-1")
    )
    ...
    asyncio.get_event_loop().run_until_complete(
        adk_runtime.run_adk(runner, "u", "anon")
    )
