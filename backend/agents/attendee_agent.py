...
Uses Google ADK + Gemini 3 Flash when credentials exist. The Concierge is a
...
def routing_sub_agent(kind: str, start_zone_id: str = "") -> dict[str, Any]:
...
def forecast_sub_agent(zone_id: str, horizon_minutes: int = 5) -> dict[str, Any]:
...
_attendee_tools: list[Callable[..., Any]] = [
...
def _fallback(message: str, location: str | None) -> dict[str, Any]:
    ...
    def record(name: str, args: dict[str, Any], result: object) -> None:
    ...
    if "forecast" in m or "in 5" in m or "later" in m:
        ...
        record("forecast_sub_agent", {"zone_id": worst["id"], "horizon_minutes": 5}, fc)
        return {
            "reply": (
    ...
    if kind in ("food", "restroom", "merch"):
        ...
        record("routing_sub_agent", {"kind": kind, "start_zone_id": location or ""}, route)
        if route.get("error"):
            return {"reply": f"Routing error: {route['error']}", "tool_calls": calls, "engine": "fallback"}
        if not route.get("dest"):
            return {"reply": "No matching zones found.", "tool_calls": calls, "engine": "fallback"}
    ...
    return {
        "reply": (
            f"Stadium-wide Flow Score is {avg}/100.{hot_s} Ask me for food, restrooms, or routes.{hint}"
        ),
        "tool_calls": calls,
        "engine": "fallback",
    }
