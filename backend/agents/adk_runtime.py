...
# We probe for the SDK at module load, but resolve the tracer *lazily* every
# time `_tool_span` is entered. This is deliberate:
#
#   Previously we did `_tracer = trace.get_tracer("flowpulse.adk")` at module
#   import time — but this module is imported during `app = FastAPI(...)`
#   construction, which runs BEFORE `configure_tracing()` fires in the lifespan
#   startup hook. The captured tracer was therefore bound to the default no-op
#   provider, and every `tool.<name>` span silently went to /dev/null.
#   Result: Cloud Trace showed zero spans despite the CloudTraceSpanExporter
#   being correctly wired up. That silently broke the "tool-span flamegraphs"
#   claim end-to-end. Lazy resolution fixes it: each span creation picks up
#   whatever provider is currently registered globally.
...
try:  # pragma: no cover
    ...
    log.debug("otel.disabled", extra={"reason": str(_otel_err)})


@contextmanager
def _tool_span(name: str, args: dict[str, Any]) -> Any:
    """Start a child span for a tool call with standard attributes.

    Resolves the tracer lazily so it picks up the provider installed by
    `configure_tracing()` during FastAPI's lifespan startup — which happens
    AFTER this module is imported. See the module-level comment above.
    """
    if _otel_trace is None:
        yield None
        return
    tracer = _otel_trace.get_tracer("flowpulse.adk")  # lazy — reads current global provider
    ...
    try:
        ...
        span.set_attribute("tool.args_hash", hashlib.sha256(args_repr.encode()).hexdigest()[:16])
        ...
        if event.content and event.content.parts:
            for p in event.content.parts:
                ...
                if fc is not None:
                    ...
                    # Reconstruct the tool-call span with its real duration.
                    # We captured `_span_started` when the function_call event
                    # fired above; compute elapsed here and stuff it in as a
                    # single tool.<name> span with `tool.duration_ms`. This
                    # makes every tool call visible as its own span in Cloud
                    # Trace (not just the zero-duration marker we used before).
                    start_ts = slot.pop("_span_started", None)
                    dur_ms = round((time.monotonic() - start_ts) * 1000.0, 2) if start_ts else 0.0
                    with _tool_span(fr.name, slot.get("args", {})) as span:
                        if span is not None and start_ts is not None:
                            span.set_attribute("tool.duration_ms", dur_ms)
                    log.info(
                        "agent.tool_end",
                        extra={
                            "agent": agent_name,
