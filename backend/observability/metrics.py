...
def write_tick_metric(avg_score: float, critical: int, congested: int,
                      zones: int) -> None:
    ...
    try:  # pragma: no cover
        ...
        def make_series(metric_type: str, value: float, unit: str = "1") -> Any:
        ...
        client.create_time_series(
            name=project_name,
            time_series=[
                make_series("custom.googleapis.com/flowpulse/crowd_flow_score",
                            avg_score),
                make_series("custom.googleapis.com/flowpulse/critical_zones",
                            float(critical)),
                make_series("custom.googleapis.com/flowpulse/congested_zones",
                            float(congested)),
            ],
        )
    except Exception as e:
