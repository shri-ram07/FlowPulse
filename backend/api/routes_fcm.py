...
@router.post("/push", dependencies=[Depends(rate_limit(RATE_LIMIT_FCM_PUSH_PER_MIN))])
async def push(p: PushPayload, _user: StaffToken = Depends(require_staff)) -> dict[str, Any]:
    ...
    if not project or creds is None:
        ...
        audit("fcm.push", actor=_user.sub, action="push_dry_run", target=topic, msg_id=msg_id)
        return {
            "ok": True,
            "dry_run": True,
            "message_id": msg_id,
    ...
    if r.status_code >= 400:
        ...
        audit("fcm.push", actor=_user.sub, action="push", target=topic, result=f"rejected_{r.status_code}")
        raise HTTPException(502, f"fcm_rejected: {r.status_code}")
    j = r.json()
    audit("fcm.push", actor=_user.sub, action="push", target=topic, message_id=j.get("name"))
